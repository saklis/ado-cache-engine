...
namespace AdoCache
{
    ...
    public class AdoCacheItem<TEntity> where TEntity : AdoCacheEntity, new()
    {
        ...
        public AdoCacheItem(string connectionString, AdoCacheItemOptions options)
        {
            ...
            {
                FieldInfo field =
                    newValueFields.Single(f => f.GetCustomAttribute<NewValueFieldAttribute>().PropertyName == column.Name);
        ...
        /// </summary>
        protected ConcurrentDictionary<string, ConcurrentDictionary<object, TEntity>> _dictionaries =
            new ConcurrentDictionary<string, ConcurrentDictionary<object, TEntity>>();
        ...
        /// </summary>
        protected ConcurrentDictionary<string, ConcurrentDictionary<object, List<TEntity>>> _indexes =
            new ConcurrentDictionary<string, ConcurrentDictionary<object, List<TEntity>>>();
        ...
        ///     Thrown when model for this item don't have property named same as
        ///     <see cref="nameOfColumn" />.
        /// </exception>
        ...
        {
            if (_dictionaries.ContainsKey(nameOfColumn))
            {
                throw new ArgumentException($"Dictionary for {nameOfColumn} already exists.");
            }
            ...
            if (!ReferenceEquals(_dictionaries.GetOrAdd(nameOfColumn, newDictionary), newDictionary))
            {
                throw new ArgumentException($"Dictionary for {nameOfColumn} already exists.");
            }
            ...
            {
                newDictionary.TryAdd(property.GetValue(entity), entity);
            }
        }
        ...
        public virtual void BuildIndex(string nameOfColumn)
        {
            ...
            if (!ReferenceEquals(_indexes.GetOrAdd(nameOfColumn, newIndex), newIndex))
            {
                throw new ArgumentException($"Index for {nameOfColumn} already exists.");
            }
        ...
        public void LoadAll()
        {
            ...
            {
                throw new
                    InvalidOperationException("Cache data has already been loaded. Call Reload() to refresh data or Unload() to clear data from cache");
            }
            ...
            where TRelation : AdoCacheEntity, new()
        {
            ...
            {
                throw new
                    InvalidOperationException("Cache data has already been loaded. Call Unload() to refresh data or Unload to clear data from cache");
            }
        ...
        public void Reload()
        {
            if (Monitor.TryEnter(_isLoading))
            {
                ...
                try
                {
                    ...
                    {
                        foreach (List<TEntity> list in index.Values)
                        {
                            list.Clear();
                        }
                    ...
                    foreach (ConcurrentDictionary<object, TEntity> dictionary in _dictionaries.Values)
                    {
                        dictionary.Clear();
                    }
        ...
        ///     - OR -
        ///     Thrown when there was unexpected result from DELETE operation on db engine - number of affected rows was different
        ///     than 1.
        /// </exception>
        /// <exception cref="ArgumentOutOfRangeException">
        ...
        public virtual void Delete(TEntity entity)
        {
            ...
            {
                throw new
                    InvalidOperationException($"Type {typeof(TEntity)} do not have any public Properties marked with Key attribute. Please mark at least one of the properties as [Key].");
            }
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, conn))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        FieldInfo field =
                            typeof(TEntity).GetField("_isManagedByCacheEngine", BindingFlags.Instance | BindingFlags.NonPublic);
                        if (field != null)
                        {
                    ...
                    {
                        throw new
                            InvalidOperationException($"There was an unexpected result while Deleting data from data base. Number of affected rows: {rowsAffected}");
                    }
                }
        ...
        public virtual TEntity Insert(TEntity entity)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                using (SqlCommand insert = BuildInsertCommand(entity, conn))
                {
                    ...
                    if (_autoIncrementColumns.Length > 0)
                    {
                        try
                        {
                            ...
                            if (insertedId > 0)
                            {
                                if (_autoIncrementColumns != null && _autoIncrementColumns.Length == 1)
                                {
                                    _autoIncrementColumns[0].SetValue(newEntity, insertedId);
                                }
                            }
                            else
                            {
                                throw new
                                    InvalidOperationException($"There was an unexpected result while Inserting data to data base. Returned index: {insertedId}");
                            }
                        }
                    ...
                    foreach (PropertyInfo keyColumn in _keyColumns.Where(c => !_autoIncrementColumns.Contains(c)))
                    {
                        keyColumn.SetValue(newEntity, keyColumn.GetValue(entity));
                    }
                    ...
                    {
                        foreach (PropertyInfo info in _columns.Except(_keyColumns).Except(_autoIncrementColumns))
                        {
        ...
        ///     - OR -
        ///     Thrown when there was an unexpected result for UPDATE operation on db engine - different number affected rows than
        ///     1.
        /// </exception>
        ...
        private string BuildDeleteQuery(TEntity entity)
        {
            ...
            foreach (PropertyInfo key in _keyColumns)
            {
                whereClauses.Add(key.Name + " = '" + key.GetValue(entity) + "'");
            }
        ...
        private string BuildSelectReadOnlyColumnsQuery(TEntity entity)
        {
            ...
            string query =
                $"SELECT {string.Join(", ", _readOnlyColumns.Select(c => c.Name).ToArray())} FROM {TableName}";
            List<string> whereClauses = new List<string>(_keyColumns.Length);
            foreach (PropertyInfo key in _keyColumns)
            {
                whereClauses.Add(key.Name + " = '" + key.GetValue(entity) + "'");
        ...
        private void DeleteFromIndexes(TEntity entity)
        {
            ...
            {
                dictionary.Value.TryRemove(typeof(TEntity).GetProperty(dictionary.Key).GetValue(entity), out TEntity _);
            }
        }
...
>>>>>>> 1500bfa1244e9d6ce2de0eec973bb220de0e345f
        ...
        private void InsertIntoIndexes(TEntity entity)
        {
            ...
            foreach (KeyValuePair<string, ConcurrentDictionary<object, TEntity>> dictionary in _dictionaries)
            {
                dictionary.Value.TryAdd(typeof(TEntity).GetProperty(dictionary.Key).GetValue(entity), entity);
            }
        }
            ...
            using (SqlCommand select = new SqlCommand(selectQuery, conn))
            {
                ...
                if (selectReader.HasRows)
                {
                    ...
                    while (selectReader.Read())
                    {
                        ...
                        {
                            throw new
                                InvalidOperationException("There was an error while querying values of ReadOnly columns. Query returned more than 1 result.");
                        }
                ...
                {
                    throw new
                        InvalidOperationException("There was an error while querying values of ReadOnly columns. Query returned no results.");
                }
            }
        }
